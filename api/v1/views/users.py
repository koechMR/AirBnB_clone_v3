#!/usr/bin/python3
'''
Create new view for User objects
'''
...
def create_user():
    ...
    # Create new User object with the JSON data
    user = User(**data)
    # Saves User object to storage
    user.save()
    # Returns newly created User object in JSON format with 201 status code
...
def update_user(user_id):
    ...
    Update User object
    '''
    # Gets User object with the given ID from the storage
    user = storage.get(User, user_id)
    if user:
        # Check if the request data is in JSON format
        if not request.get_json():
        ...
        # Update attributes of User object with the JSON data
        for key, value in data.items():
            if key not in ignore_keys:
                setattr(user, key, value)

        # Save updated User object to the storage
        user.save()
